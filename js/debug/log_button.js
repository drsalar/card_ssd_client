...
// 设计要点：
//   1. 房间场景的“退出”按钮位于 (SCREEN_WIDTH-60, 10, 50, 28)，本按钮位于其左侧 8px；
//   2. 大厅/其他场景退出按钮不存在时，按钮直接靠右；
...
const BTN_W = 44;
const BTN_H = 28;
const RIGHT_GAP = 8;        // 与画布右边缘距离
const TOP_GAP = 10;         // 距顶部
const EXIT_BTN_W = 50;      // 房间退出按钮宽度（与 room_scene 中保持一致）
const EXIT_BTN_RIGHT_GAP = 60; // 退出按钮右边距 (SCREEN_WIDTH - 60)
...
export default class LogButton {
  ...
  // 计算按钮位置 x：若处于房间场景，则避让退出按钮；否则贴右
  _computeRect() {
    let x = SCREEN_WIDTH - this.w - RIGHT_GAP;
    try {
      const databus = GameGlobal.databus;
      if (databus && databus.scene === 'room') {
        // 退出按钮位于 SCREEN_WIDTH - 60，宽 50；LOG 按钮放在退出按钮左侧 8px
        x = (SCREEN_WIDTH - EXIT_BTN_RIGHT_GAP) - this.w - RIGHT_GAP;
      }
    } catch (e) {}
    return { x, y: TOP_GAP, w: this.w, h: this.h };
  }
